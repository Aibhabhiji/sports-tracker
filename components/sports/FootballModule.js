...
export default function FootballModule({ participants, sportState, onUpdateSportState }) {
  ...
  const handleCreateTeam = (e) => {
    ...
    const newTeam = {
      ...
      players: [] // { id, name, role: 'Forward'|'Mid Field'|'Defender'|'Goal Keeper', goals: 0, soldPrice: 0 }
    };
        ...
        return {
          ...
          players: [
            ...t.players,
            {
              id: playerObj.id || playerObj.regId || playerObj.Registration_ID,
              name: playerObj.name,
              role: 'Forward', // Default role upon auction win
              goals: 0,
              soldPrice: price
            }
          ]
  ...
  const handleUpdateMatchScore = (matchId, teamAId, teamBId, teamAScore, teamBScore, scorerEvents) => {
  ...
  if (isGrandFinaleCompleted && finaleMatch) {
    if (finaleMatch.teamAScore > finaleMatch.teamBScore) {
      ...
      finaleWinner = 'Draw / Penalty Shootout Winner';
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Sub-navigation tabs for Football */}
...
function MatchScoringCard({ match, teamA, teamB, onSaveMatch, onDeleteMatch }) {
